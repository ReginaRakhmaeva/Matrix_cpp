...
TEST(S21MatrixTest, DefaultConstructor) {
  ...
  EXPECT_EQ(m.getMatrix(), nullptr);  // Проверка, что матрица не аллоцирована
}
...
TEST(S21MatrixTest, ParameterizedConstructor) {
  ...
  EXPECT_NE(m.getMatrix(), nullptr);  // Проверка, что память выделена
  EXPECT_EQ(m.getValue(0, 0),
            0.0);  // Проверка, что элементы инициализированы нулями
}

// Тестирование конструктора копирования
TEST(S21MatrixTest, CopyConstructor) {
  S21Matrix original(2, 3);
  original.setValue(0, 0, 5.0);
  S21Matrix copy(original);
  EXPECT_EQ(copy.getRows(), 2);
  EXPECT_EQ(copy.getCols(), 3);
  EXPECT_EQ(copy.getValue(0, 0),
            5.0);  // Проверка, что значения скопированы правильно
}
...
TEST(S21MatrixTest, MoveConstructor) {
  ...
  EXPECT_EQ(moved.getValue(0, 0), 5.0);
  EXPECT_EQ(original.getMatrix(),
